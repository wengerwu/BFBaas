...
import (
	...
	"github.com/paybf/baasmanager/baas-core/common/fileutil"
	...
	"github.com/paybf/baasmanager/baas-core/common/util"
	"github.com/paybf/baasmanager/baas-core/core/fasdk"
	"github.com/paybf/baasmanager/baas-core/core/model"
	"github.com/paybf/baasmanager/baas-fabricengine/config"
	"github.com/paybf/baasmanager/baas-fabricengine/constant"
	"github.com/paybf/baasmanager/baas-fabricengine/fautil"
	"github.com/paybf/baasmanager/baas-fabricengine/generate"
)
...
func (f FabricService) queryBlocksPage(ctx *gin.Context) {
	var chain model.FabricChain
	if err := ctx.ShouldBindJSON(&chain); err != nil {
		gintool.ResultFail(ctx, err)
		return
	}
	pageStart, err := strconv.Atoi(ctx.Query("pageStart"))
	if err != nil {
		gintool.ResultFail(ctx, err)
		return
	}
	...
	start := int(height) - pageStart - 1
	end := start - limit
	for i := start; i > end; i-- {
		block, err := fsdk.QueryBlock(uint64(i))
		if err != nil {
			gintool.ResultFail(ctx, err)
			return
		}
		...
		for j := 0; j < len(block.Transactions); j++ {
			block.Transactions[j].Actions = nil
			block.Transactions[j].Config = ""
		}
...
func Server() {
	...
	router.POST("/queryBlocksPage", fabric.queryBlocksPage)
